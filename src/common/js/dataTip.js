...
window.ZENG.dom = {
  ...
  setStyle: function(el, properties, value) {
    if (!(el = window.ZENG.dom.get(el)) || el.nodeType != 1) {
      return false;
    }
    var tmp,
      bRtn = true,
    ...
    for (var prop in properties) {
      ...
      if (prop == 'float') {
        ...
        if (!w3cMode) {
          prop = 'filter';
          value =
            value >= 1 ? '' : 'alpha(opacity=' + Math.round(value * 100) + ')';
        }
      } else if (
        ...
        if (w3cMode) {
          var v = window.ZENG.dom.getStyle(el, 'backgroundPosition' + tmp);
...
window.ZENG.string = {
  ...
  format: function(str) {
    ...
    return str.replace(window.ZENG.string.RegExps.format, function(m, n) {
...
window.ZENG.object = {
  ...
  route: function(obj, path) {
    obj = obj || {};
    path = String(path);
    var r = window.ZENG.object.routeRE,
      m;
    r.lastIndex = 0;
...
window.ZENG.msgbox.show = function(msgHtml, type, timeout, opts) {
  if (typeof opts == 'number') {
    opts = { topPosition: opts };
  }
  opts = opts || {};
  var _s = window.ZENG.msgbox,
    template =
...
window.ZENG.msgbox._setPosition = function(tips, timeout, topPosition) {
  timeout = timeout || 5000;
  var _s = window.ZENG.msgbox,
    bt = window.ZENG.dom.getScrollTop(),
    ch = window.ZENG.dom.getClientHeight(),
...
window.ZENG.msgbox.hide = function(timeout) {
  var _s = window.ZENG.msgbox;
  if (timeout) {
    clearTimeout(_s._timer);
...
window.ZENG.msgbox._hide = function() {
  var _mBox = window.ZENG.dom.get('q_Msgbox'),
    _s = window.ZENG.msgbox;
  clearTimeout(_s._timer);
  if (_mBox) {
...
var dataTip = {
  succTip: function(mess) {
    var _this = this;
    _this.msgbox(4, mess);
  },
  infoTip: function(mess) {
    var _this = this;
    _this.msgbox(1, mess);
  },
  errorTip: function(mess) {
    var _this = this;
    _this.msgbox(5, mess);
  },
  loadingTip: function(mess) {
    window.ZENG.msgbox.show(mess, 6, 10000);
  },
  hideTip: function() {
    window.ZENG.msgbox._hide();
  },
  msgbox: function(i, tip) {
